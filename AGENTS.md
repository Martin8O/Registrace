...
npm test                    # Vitest (350 tests, no database needed)
npm run lint                # ESLint
...
- **An event's stored status lags the calendar — on purpose.** Public visibility is derived on
  ...
  three days later) — hours behind in the normal case, and until the next successful run if one
  ...
  wait for the column. Once CLOSED/ARCHIVED, the admin events list hides the Edit link (a B6
  decision the cron makes real) and the registrations list hides ARCHIVED events' rows behind
  its "show archived" switch; registration editing, resend and export do not look at it.
